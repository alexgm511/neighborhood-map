            ...
            if (self.neighborhood().length == 0) {
                ...
                self.geocoder.geocode({
                    ...
                    if (status == google.maps.GeocoderStatus.OK) {						
                        ...
                        self.markersHide(true);
						// Changed from using named variables to built-in function lat() and lng()
                        self.fsqData.fsqCallback(self.location.lat(), self.location.lng());
                   } else {
        ...
        showPlace: function() {
            ...
            for (var i = 0; i < self.markers.length; i++) {
                ...
                if (self.markers[i].id === this.id) {
					...
					// Changed from using named variables to built in functions getNorthEast() and lat()
					var northEast = bounds.getNorthEast();
                    var newCenter = parseFloat(this.location.lat) + ((parseFloat(northEast.lat()) - parseFloat(this.location.lat))*parseFloat(0.6));
